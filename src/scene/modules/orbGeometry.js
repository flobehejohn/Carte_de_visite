...
function ensureOrbShellConfig(ctx) {
  ...
  // guards
  ctx.orbShellConfig.radius = Math.max(0.1, Number(ctx.orbShellConfig.radius || 1.8));
  ctx.orbShellConfig.detail = Math.max(0, Math.floor(ctx.orbShellConfig.detail ?? 1));
  ctx.orbShellConfig.wireLayers = Math.max(1, Math.min(8, Math.floor(ctx.orbShellConfig.wireLayers ?? 3)));
  ctx.orbShellConfig.wireSpacing = Math.max(0.02, Math.min(0.14, Number(ctx.orbShellConfig.wireSpacing ?? 0.06)));
  ctx.orbShellConfig.wireBreath = Math.max(0.05, Math.min(0.65, Number(ctx.orbShellConfig.wireBreath ?? 0.25)));
  ctx.orbShellConfig.wireOpacity = Math.max(0, Math.min(1, Number(ctx.orbShellConfig.wireOpacity ?? 0.15)));
  ctx.orbShellConfig.wireOpacityInner = Math.max(0, Math.min(1, Number(ctx.orbShellConfig.wireOpacityInner ?? 0.08)));
  ctx.orbShellConfig.wireHueShift = Math.max(0, Math.min(0.2, Number(ctx.orbShellConfig.wireHueShift ?? 0.02)));
...
function buildGeometryFromConfig(cfg) {
  ...
  const d = Math.max(0, Math.floor(cfg.detail)); // allow 0

  // NOTE: quelques alias "simplices"
  switch (cfg.shapeType) {
    case 'sphere': return new THREE.SphereGeometry(r, 32 + d * 16, 20 + d * 10);
    case 'capsule': return new THREE.CapsuleGeometry(r * 0.72, r * 0.8, 10 + d * 6, 18 + d * 10);
    case 'cone': return new THREE.ConeGeometry(r * 0.9, r * 1.8, 22 + d * 10, 1, true);
    case 'box': return new THREE.BoxGeometry(r * 1.5, r * 1.5, r * 1.5, 4 + d * 4, 4 + d * 4, 4 + d * 4);
    case 'torus': return new THREE.TorusGeometry(r * 0.8, r * 0.3, 22 + d * 12, 44 + d * 28);
    case 'torusKnot': return new THREE.TorusKnotGeometry(r * 0.7, r * 0.2, 120 + d * 64, 14 + d * 10, cfg.knotP || 2, cfg.knotQ || 3);
    case 'octaDetail': return new THREE.OctahedronGeometry(r, Math.max(2, d + 2));
    case 'octa': return new THREE.OctahedronGeometry(r, d);
    case 'tetra': return new THREE.TetrahedronGeometry(r, d);
    case 'dodeca': return new THREE.DodecahedronGeometry(r, d);
    case 'icosa':
...
export function setRitualConfig(ctx, genome) {
  ...
  ctx.orbShellConfig.noiseFrequency1 = g.noise?.f1 ?? ctx.orbShellConfig.noiseFrequency1;
  ctx.orbShellConfig.noiseFrequency2 = g.noise?.f2 ?? ctx.orbShellConfig.noiseFrequency2;
  ctx.orbShellConfig.noiseFrequency3 = g.noise?.f3 ?? ctx.orbShellConfig.noiseFrequency3;

  ctx.orbShellConfig.baseDeformAmplitude = g.baseDeform ?? ctx.orbShellConfig.baseDeformAmplitude;
  ctx.orbShellConfig.pulseAmplitude = g.pulseDeform ?? ctx.orbShellConfig.pulseAmplitude;
  ctx.orbShellConfig.dislocation = g.dislocation ?? ctx.orbShellConfig.dislocation;

  ctx.orbShellConfig.wireLayers = g.wire?.layers ?? ctx.orbShellConfig.wireLayers;
  ctx.orbShellConfig.wireSpacing = g.wire?.spacing ?? ctx.orbShellConfig.wireSpacing;
  ctx.orbShellConfig.wireBreath = g.wire?.breath ?? ctx.orbShellConfig.wireBreath;
  ctx.orbShellConfig.wireOpacity = g.wire?.opacityBase ?? ctx.orbShellConfig.wireOpacity;
  ctx.orbShellConfig.wireOpacityInner = g.wire?.opacityInner ?? ctx.orbShellConfig.wireOpacityInner;
...
export function createPolyhedron(ctx) {
  ...
  const material = ctx.ensureOrbMaterial
    ...
    : new THREE.MeshStandardMaterial({ color: 0x222222, roughness: 0.45, metalness: 0.05 });
 ...
 */
export function updateWireframeStyle(ctx, baseColor, baseOpacity, time = 0, turbulence = 0.2) {
  ...
  for (const w of ctx.wireFrames) {
    ...
    // respiration
    const breath = Math.sin(time * (0.6 + cfg.wireBreath) + phase) * (0.08 + turbulence * 0.12);
    const micro = Math.sin(time * (2.1 + i * 0.15) + phase * 1.7) * (0.03 + turbulence * 0.08);
...
export function setDeformAmplitude(ctx, { base, pulse, dislocation }) {
  ensureOrbShellConfig(ctx);
  ctx.orbShellConfig.baseDeformAmplitude = Number(base ?? ctx.orbShellConfig.baseDeformAmplitude);
  ctx.orbShellConfig.pulseAmplitude = Number(pulse ?? ctx.orbShellConfig.pulseAmplitude);
  if (dislocation !== undefined) ctx.orbShellConfig.dislocation = Number(dislocation ?? ctx.orbShellConfig.dislocation);
}
...
export function deformPolyhedron(ctx, time = 0) {
  ...
  const pulseFactor = Math.sin(time * 1.8) * (cfg.pulseAmplitude || 0);
  const baseAmp = cfg.baseDeformAmplitude || 0;
  const disl = cfg.dislocation || 0;
  ...
  for (let i = 0; i < count; i++) {
    ...
    // 3 couches de bruit (Simplex)
    const n1 = simplex.noise4d(ox * f1, oy * f1, oz * f1, time * 0.18);
    const n2 = simplex.noise4d(ox * f2 + 11.3, oy * f2, oz * f2, time * 0.12);
    const n3 = simplex.noise4d(ox * f3, oy * f3 + 17.7, oz * f3, time * 0.09);
