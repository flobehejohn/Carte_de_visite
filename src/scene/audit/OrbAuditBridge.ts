...
function preserveAuditBridgeFunctions(
  ...
  for (const key of [
    ...
    'scanFeedbackCandidates',
  ]) {
    if (typeof root[key] === 'function') {
      preservedFns[key] = root[key];
    }
  }
...
function ensureAuditRoot(): OrbAuditRoot | null {
  ...
  if (!isPlainObject(existing)) {
    window.__ORB_AUDIT__ = preserveAuditBridgeFunctions(window.__ORB_AUDIT__, base);
    return window.__ORB_AUDIT__;
  }

  window.__ORB_AUDIT__ = preserveAuditBridgeFunctions(window.__ORB_AUDIT__, {
    ...base,
    ...existing,
    qualityProfiles: {
...
export class OrbAuditBridge {
  ...
  constructor(ctx: any) {
    this.ctx = ctx?.ctx ?? ctx;
    ensureAuditRoot();
  }

  hookIntoRenderer(): void {
    OrbAuditBridge.ensureInitialized();
  }

  captureRuntimeState(): void {
