...
function fallbackOracle(ritual: RitualInput): OracleResult {
  ...
  const interpretation =
    'Sans reponse exploitable, l oracle tisse un motif interieur.';
  const textLength = `${quote}${interpretation}`.length;
...
function extractJsonFromAny(text: string, jsonField: unknown): any | null {
  if (jsonField && typeof jsonField === 'object') return jsonField as any;
  ...
  return extractFirstJsonObject(String(text ?? '')) ?? null;
}

async function callOracleOnce(
  ritual: RitualInput,
  opts?: OracleOptions,
  retryReason?: string,
): Promise<{ data: any | null; rawText: string; traceId: string }> {
  const prompt = buildOraclePrompt(
    ritual,
    opts?.climateSnapshot ?? null,
    retryReason ? { retryReason } : undefined,
  );
  const traceId = opts?.traceId ? String(opts.traceId) : `ui_${Date.now()}`;

  const r = await geminiGenerate([ORACLE_SYSTEM_PROMPT, prompt].join('\n\n'), {
    traceId,
    expectJson: true,
    temperature: 0.7,
    topP: 0.9,
  ...
  const data = extractJsonFromAny(r.text, r.json);
  return { data, rawText: r.text, traceId };
}
...
export async function consultOracle(
  ...
  try {
    const first = await callOracleOnce(ritual, opts);
    if (!first.data) {
      logger.warn('Oracle response not JSON; fallback.', first.traceId);
    ...
    if (guard.shouldRetry(combined)) {
      const retry = await callOracleOnce(ritual, opts, 'EN_LIKELY');
      if (retry.data) {
        result = buildOracleResult(ritual, retry.data);
      } else {
        logger.warn('Oracle retry not JSON; keeping first.', retry.traceId);
      }
    }

    return result;
  } catch (e) {
    logger.warn('Oracle error:', e);
    return fallbackOracle(ritual);
  }
}

/**
 * Guardian minimal (si tu l’utilises)
 * Renvoie toujours quelque chose de safe.
 */
export async function getStepGuidance(
  ...
  const system = [
    ...
    'LANGUE: francais uniquement. Aucun anglais, aucun franglais.',
    'SORTIE: JSON uniquement. Aucun markdown. Aucun texte hors JSON.',
    'JSON_SCHEMA: { "comment":"string", "isSafe":boolean }',
  ].join('\n');

  const prompt = [
    `STEP: ${toAscii(step)}`,
    `CHOICE: ${toAscii(value)}`,
    'SORTIE: JSON uniquement. Aucun markdown. Aucun texte hors JSON.',
  ].join('\n');

  try {
    const r = await geminiGenerate([system, prompt].join('\n\n'), {
      traceId,
      expectJson: true,
      temperature: 0.2,
      topP: 0.9,
    ...
    const data = extractJsonFromAny(r.text, r.json);
    if (!data) return { comment: 'Le silence repond...', isSafe: true };

    let comment = String((data as any).comment ?? 'Le silence repond...');
    let isSafe = Boolean((data as any).isSafe ?? true);

    if (guard.shouldRetry(comment)) {
      const r2 = await geminiGenerate(
        [system, prompt, ORACLE_RETRY_FR].join('\n\n'),
        {
          traceId: `${traceId}_r`,
          expectJson: true,
          temperature: 0.2,
      ...
      const d2 = extractJsonFromAny(r2.text, r2.json);
      if (d2) {
        comment = String((d2 as any).comment ?? comment);
        isSafe = Boolean((d2 as any).isSafe ?? isSafe);
      }
    }
