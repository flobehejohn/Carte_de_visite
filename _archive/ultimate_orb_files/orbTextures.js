  ...
  if (enabled) {
    ...
    if (!material.userData.__chromaKey) {
      ...
      material.onBeforeCompile = shader => {
        ...
        // Lier les uniforms GLSL aux valeurs JS
        ...
        // S'assurer que les uniforms sont déclarés dans le shader
        ...
        // Injection du code chroma juste après le dithering
...
export function setBackgroundVideo(ctx, videoEl) {
  ...
  // Three.js r152+ : encoding -> colorSpace
  if ('colorSpace' in videoTex) {
    videoTex.colorSpace = THREE.SRGBColorSpace;
  } else {
    // @ts-ignore
...
export function applyRitualTextureTuning(ctx) {
  ...
  if (palette?.accent?.isColor) {
    // On utilise l'accent comme "keyColor" si l'utilisateur veut un chroma créatif.
