...
function ensureConfig(ctx) {
  ...
  // normalize colors
  if (!ctx.particlesConfig.color1?.isColor) ctx.particlesConfig.color1 = new THREE.Color(ctx.particlesConfig.color1 || 0xffffff);
  if (!ctx.particlesConfig.color2?.isColor) ctx.particlesConfig.color2 = new THREE.Color(ctx.particlesConfig.color2 || 0xffaa00);
  ...
  ctx.particlesConfig.opacity = Math.max(0, Math.min(1, Number(ctx.particlesConfig.opacity ?? 0.55)));
  ...
  ctx.particlesConfig.dynamics.lfoSpeed = Math.max(0.03, Math.min(0.6, Number(ctx.particlesConfig.dynamics.lfoSpeed ?? 0.14)));
  ctx.particlesConfig.dynamics.maxNeighbors = Math.max(8, Math.min(64, Math.floor(ctx.particlesConfig.dynamics.maxNeighbors ?? 28)));
...
export function createInnerParticles(ctx) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    basePos[idx] = x; basePos[idx + 1] = y; basePos[idx + 2] = z;
  ...
  const linkMaterial = new THREE.LineBasicMaterial({
    color: cfg.color1,
    transparent: true,
    opacity: Math.min(1, cfg.opacity * 0.45),
    depthWrite: false,
    blending: THREE.AdditiveBlending
  });
...
export function animateParticles(ctx, time, turbulence = 0.25) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const burst = burstBoost * (0.65 + 0.35 * Math.sin(time * 6.0 + ph)) * (0.35 + 0.65 * Math.abs(si));
...
export function updateParticleLinks(ctx) {
  ...
  const lfo = 0.5 + 0.5 * Math.sin((ctx?.ritualGenome?.progress ?? 0.5) * Math.PI * 2 + performance.now() * 0.0004);
  ...
  for (let i = 0; i < count; i++) {
    const idxA = i * 3;
    const ax = pPos[idxA], ay = pPos[idxA + 1], az = pPos[idxA + 2];
    ...
    for (const dx of neighbors) for (const dy of neighbors) for (const dz of neighbors) {
      ...
      for (let k = 0; k < list.length; k++) {
        ...
        const idxB = j * 3;
        const bx = pPos[idxB], by = pPos[idxB + 1], bz = pPos[idxB + 2];
        ...
        lPos[write++] = ax; lPos[write++] = ay; lPos[write++] = az;
        lPos[write++] = bx; lPos[write++] = by; lPos[write++] = bz;
  ...
  // opacité modulée (lente)
  const slow = 0.5 + 0.5 * Math.sin(performance.now() * 0.0007);
  ctx.particlesLinks.material.opacity = Math.min(1, cfg.opacity * (0.22 + slow * 0.28));
...
export function updateParticleTrails(ctx) {
  ...
  ctx.particlesTrails.material.opacity = cfg.opacity * 0.9;
...
export function setParticlesConfig(ctx, patch = {}) {
  ...
  if (ctx.particlesPoints?.material) {
    ...
    ctx.particlesPoints.material.opacity = cfg.opacity;
  ...
  if (ctx.particlesTrails?.material) {
    ...
    ctx.particlesTrails.material.opacity = cfg.opacity;
    ctx.particlesTrails.material.needsUpdate = true;
  }
  if (ctx.particlesLinks?.material) {
    ctx.particlesLinks.material.opacity = Math.min(1, cfg.opacity * 0.45);
