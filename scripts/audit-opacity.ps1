...
param(
    ...
    [string]$OutDir = "audit/_latest/opacity",
    [string]$RunStamp = "",
    [int]$Keep = 3,
    [switch]$Quiet
)
...
try {
    ...
    if (-not $script:Quiet) { Write-Host "`n=== audit opacity ===" -ForegroundColor Cyan }
    ...
    if ($exitCode -eq 0) {
        Write-Host ("[OK] audit opacity => {0}" -f $latestPath) -ForegroundColor Green
        exit 0
    }
    if ($exitCode -eq 2) {
        Write-Host ("[OK] audit opacity (warn) => {0}" -f $latestPath) -ForegroundColor Yellow
        exit 2
    }
    Write-Host ("[KO] audit opacity => {0}" -f $latestPath) -ForegroundColor Red
    exit 1
}
catch {
